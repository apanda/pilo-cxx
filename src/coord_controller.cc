...
namespace PILO {
    ...
    void Coordinator::RegisterController(CoordinationController* controller) {
    ...
    void Coordinator::receive(CoordinationController* controller, 
        ...
        if (_lastSeen[packet->_destination] < packet->_id) {
            ...
            _context->scheduleAbsolute(t, [this, packet, link](Time) mutable {
                        send_to_controller(packet, link);
                    });
        }
    }
    
    void Coordinator::send_to_controller(
            std::shared_ptr<Packet> packet, Link* link) {
        for (auto controller : _controllers) {
            CoordinationController* c = controller;
            _context->schedule(0.0, 
                    [c, packet, link](Time) mutable {
                        c->receive_coordinator(packet, link);
                    });
        }
    }

    CoordinationController::CoordinationController(Context& context,
         const std::string& name,
         const Time refresh,
         const Time gossip,
         Distribution<bool>* drop) :
        Controller(context, name, refresh, gossip, drop),
        _coordinator(Coordinator::GetInstance()) {
    ...
    void CoordinationController::receive_coordinator(std::shared_ptr<Packet> packet,
                                                     Link* link) {
